...
namespace {
    ...
    TEST_F(PythonEnricherTest, ReportsDeclaredEnricherType)
    {
        {
            std::ofstream script(script_path_);
            script << "ENRICHER_TYPE = 'tag_payload'\n" "def enrich(batch):\n" "    return batch\n";
        }

        PythonEnricher enricher(makeConfigFromYaml("script-path: " + script_path_.string()));
        EXPECT_EQ("tag_payload", enricher.enricherType());
    }

    TEST_F(PythonEnricherTest, RejectsNonStringEnricherType)
    {
        std::ofstream script(script_path_);
        script << "ENRICHER_TYPE = 42\n" "def enrich(batch):\n" "    return batch\n";

        EXPECT_THROW(PythonEnricher(makeConfigFromYaml("script-path: " + script_path_.string())), std::runtime_error);
    }
