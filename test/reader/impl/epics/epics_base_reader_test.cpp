...
pvs:
  - name: test:status
    ...
    {
        source_events = countEventsForSource(*mock_bus, "test:status");
        const auto snapshot = snapshotter.getSnapshot(*metrics);
        const auto pushes = findReaderPushesForPv(snapshot, "test:status");
        matched_metrics = pushes.has_value() && source_events >= 2 &&
                          pushes.value() == static_cast<long long>(source_events);
        if (matched_metrics)
            break;
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        waited_ms += 100;
    ...
    EXPECT_GE(source_events, 2u) << "Expected multiple events for test:status";
    EXPECT_TRUE(matched_metrics) << "Expected metrics snapshot pushes to match received events for test:status";
    reader_ptr.reset();
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
}
...
pvs:
  ...
  - name: test:table
    ...
    // test:voltage (NTScalar<Float64>) -> double column
    {
