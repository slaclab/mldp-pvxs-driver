...
TEST_F(PeriodicMetricsDumperTest, EscapesQuotesInMetricLabels)
{
    ...
    metrics.incrementWriterPushes(1.0, {{"reader", "pv\"name"}});
    ...
    EXPECT_NE(jsonl.find("\"mldp_pvxs_driver_writer_push_total\""), std::string::npos);
...
TEST_F(PeriodicMetricsDumperTest, GroupsHistogramSamplesUnderBaseMetric)
{
    ...
    EXPECT_NE(jsonl.find("\"mldp_pvxs_driver_reader_processing_time_ms\""), std::string::npos);
    EXPECT_EQ(jsonl.find("mldp_pvxs_driver_reader_processing_time_ms_bucket"), std::string::npos);
