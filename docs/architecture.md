...
- **Metrics surface**: A shared `metrics::Metrics` instance is created in the controller and passed to readers (via `ReaderFactory::create`) and pool components; it is also exposed through `metrics()` for any new layers that need to emit counters. The full set of Prometheus metrics is organized into four groups:
  - **Reader metrics** (`source=PV_NAME`): `reader_events_received_total` (raw EPICS updates from subscriptions), `reader_events_total` (successfully processed), `reader_errors_total`, `reader_processing_time_ms` (conversion histogram, milliseconds), `reader_queue_depth` (internal work queue depth), `reader_pool_queue_depth` (conversion tasks queued in the reader thread pool).
