...
namespace {
    ...
    public:
        ...
        {
            if (context().progress) context().progress->setActivity({}, "block nested-loop join");
...
std::unique_ptr<IExecutionState> mldp_pvxs_driver::query::executor::makeBlockNestedLoopJoinExecutionState(const plan::PhysicalBlockNestedLoopJoin& node, const plan::PhysicalNodePtr&, const ExecutionContext& context, QueryStats& stats)
{
