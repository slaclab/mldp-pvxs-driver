...
std::vector<std::string> shardPvs(const std::vector<Predicate>& predicates)
{
    ...
    for (const auto& predicate : predicates)
    {
        ...
        {
            if (std::holds_alternative<std::string>(value))
                pvs.push_back(std::get<std::string>(value));
        }
    }
    return pvs;
}
...
WindowBackendScanRecordBatchStream::WindowBackendScanRecordBatchStream(
    const plan::PhysicalTableScan&           scan,
    ExecutionContext                         context,
    std::shared_ptr<QueryStats>              stats,
    std::vector<std::pair<int64_t, int64_t>> windows)
    : scan_(scan), context_(std::move(context)), stats_(std::move(stats)), queryable_(QueryableFactory::instance().createByTable(scan_.table_name)), windows_(std::move(windows))
{
    if (windows_.empty())
        throw std::runtime_error("Streaming window scan requires at least one window");
    selectWindow();
    for (const auto& predicate : scan_.pushable_predicates)
    {
        ...
        for (const auto& value : predicate.values)
            if (std::holds_alternative<std::string>(value))
                requested_pvs_.push_back(std::get<std::string>(value));
    }
    if (requested_pvs_.empty())
        throw std::runtime_error("MLDP time-series window requires a PV predicate");
    prepareNextSlice();
}
...
std::shared_ptr<arrow::RecordBatch> WindowBackendScanRecordBatchStream::next()
{
    for (;;)
    {
        ...
        {
            if (context_.shard_trace)
                context_.shard_trace->fail(group.trace_entry_index, error.what());
        ...
        if (!batch)
        {
            if (context_.progress)
                context_.progress->completeShard();
            group.stream.reset();
            ++group_index_;
...
void WindowBackendScanRecordBatchStream::scheduleFirst(Group& group)
{
    ...
    group.next = std::async(std::launch::async, [this, predicates, description, shard_context = std::move(shard_context), trace, trace_entry_index]() mutable
                            {
                                try
                                {
                                    ...
                                    {
                                        if (batch)
                                            trace->recordBatch(trace_entry_index, static_cast<uint64_t>(batch->num_rows()));
                                        else
                                            trace->complete(trace_entry_index);
                                    }
                                ...
                                {
                                    if (trace)
                                        trace->fail(trace_entry_index, error.what());
...
void WindowBackendScanRecordBatchStream::scheduleNext(Group& group)
{
    ...
    const auto trace = context_.shard_trace;
    const auto trace_entry_index = group.trace_entry_index;
    group.next = std::async(std::launch::async, [description, stream = std::move(stream), trace, trace_entry_index]() mutable
                            {
                                try
                                {
                                    auto batch = stream->next();
...
void WindowBackendScanRecordBatchStream::prepareNextSlice()
{
    ...
    while (window_index_ < windows_.size())
    {
        ...
        {
            ++window_index_;
            if (window_index_ >= windows_.size())
                return;
            selectWindow();
            continue;
        }
        ...
        for (std::size_t offset = 0; offset < requested_pvs_.size(); offset += scan_.window_shards.series_per_shard)
        {
            ...
            for (std::size_t index = offset; index < end; ++index)
                values.emplace_back(requested_pvs_[index]);
        ...
        if (final_slice_)
            slice_begin_ns_ = window_end_ns_ + 1;
        else
            slice_begin_ns_ = slice_end_ns_;
        return;
    }
}
