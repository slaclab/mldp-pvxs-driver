...
Element StatusBar(const WizardState& w, bool valid, bool dirty, const std::string& msg)
{
...
static Component MakeControllerForm(WizardState* w, PanelAppState* state)
{
    ...
    auto name_field = InputField("Name", &w->controller_name, [](const std::string& s)
                                 {
...
static Component MakeMldpWriterForm(MldpWriterConfig* cfg, PanelAppState* state)
{
    ...
    // Basic Settings
    auto f_name = InputField("Name", &cfg->name, [](const std::string& s)
                             {
                                 return s.empty() ? "Must not be empty" : "";
                             },
                             [on_change]
                             ...
                             on_focus_fn("name"));
    auto f_tp = InputField("Thread Pool", &cfg->thread_pool, [](const std::string& s)
                           {
                               return isPositiveInt(s) ? "" : "Must be positive int";
                           },
                           [on_change]
                           {
                               on_change("thread_pool");
                           },
                           on_focus_fn("thread_pool"));
    auto f_smb = InputField("Stream Max Bytes", &cfg->stream_max_bytes, [](const std::string& s)
                            {
                                return isPositiveInt(s) ? "" : "Must be positive int";
                            },
                            [on_change]
                            {
                                on_change("stream_max_bytes");
                            },
                            on_focus_fn("stream_max_bytes"));
    auto f_sma = InputField("Stream Max Age ms", &cfg->stream_max_age_ms, [](const std::string& s)
                            {
                                return isPositiveInt(s) ? "" : "Must be positive int";
                            },
                            [on_change]
                            {
                                on_change("stream_max_age_ms");
                            },
                            on_focus_fn("stream_max_age_ms"));

    // MLDP Pool
    auto f_pname = InputField("Provider Name", &cfg->provider_name, [](const std::string& s)
                              {
                                  return s.empty() ? "Must not be empty" : "";
                              },
                              [on_change]
                              {
                                  on_change("provider_name");
                              },
                              on_focus_fn("provider_name"));
    auto f_pdesc = InputField("Provider Desc", &cfg->provider_desc, [](const std::string&)
                              {
                                  return "";
                              },
                              [on_change]
                              {
                                  on_change("provider_desc");
                              },
                              on_focus_fn("provider_desc"));
    auto f_iurl = InputField("Ingestion URL", &cfg->ingestion_url, [](const std::string& s)
                             {
                                 return s.empty() ? "Must not be empty" : "";
                             },
                             [on_change]
                             {
                                 on_change("ingestion_url");
                             },
                             on_focus_fn("ingestion_url"));
    auto f_qurl = InputField("Query URL", &cfg->query_url, [](const std::string&)
                             {
                                 return "";
                             },
                             [on_change]
    ...
    auto f_minc = InputField("Min Connections", &cfg->min_conn, [](const std::string& s)
                             {
                                 return isPositiveInt(s) ? "" : "Must be positive int";
                             },
                             [on_change]
    ...
    auto f_maxc = InputField("Max Connections", &cfg->max_conn, [](const std::string& s)
                             {
                                 return isPositiveInt(s) ? "" : "Must be positive int";
                             },
                             [on_change]
    ...
    for (int i = 0; i < static_cast<int>(creds_choices.size()); ++i)
        if (creds_choices[i] == cfg->creds_type)
        {
            *creds_idx = i;
            break;
        }

    auto f_cert = InputField("PEM Cert Chain", &cfg->pem_cert_chain, [](const std::string&)
                             {
                                 return "";
                             },
                             [on_change]
                             {
                                 on_change("pem_cert_chain");
                             },
                             on_focus_fn("pem_cert_chain"));
    auto f_pkey = InputField("PEM Private Key", &cfg->pem_private_key, [](const std::string&)
                             {
                                 return "";
                             },
                             [on_change]
                             {
                                 on_change("pem_private_key");
                             },
                             on_focus_fn("pem_private_key"));
    auto f_root = InputField("PEM Root Certs", &cfg->pem_root_certs, [](const std::string&)
                             {
                                 return "";
                             },
                             [on_change]
                             {
                                 on_change("pem_root_certs");
                             },
                             on_focus_fn("pem_root_certs"));

    auto tls_fields = Container::Vertical({f_cert, f_pkey, f_root});
    auto tls_maybe = Maybe(tls_fields, [creds_idx]
                           {
                               return *creds_idx == 2;
                           });
...
static Component MakeHdf5WriterForm(Hdf5WriterConfig* cfg, PanelAppState* state)
{
    ...
    auto f_name = InputField("Name", &cfg->name, [](const std::string& s)
                             ...
                             on_focus_fn("name"));
    auto f_path = InputField("Base Path", &cfg->base_path, [](const std::string& s)
                             {
                                 return s.empty() ? "Must not be empty" : "";
                             },
                             [on_change]
    ...
    auto f_age = InputField("Max File Age (s)", &cfg->max_file_age_s, [](const std::string& s)
                            {
                                return isPositiveInt(s) ? "" : "Must be positive int";
                            },
                            [on_change]
                            {
                                on_change("max_file_age_s");
                            },
                            on_focus_fn("max_file_age_s"));
    auto f_size = InputField("Max File Size (MB)", &cfg->max_file_size_mb, [](const std::string& s)
                             {
                                 return isPositiveInt(s) ? "" : "Must be positive int";
                             },
                             [on_change]
                             {
                                 on_change("max_file_size_mb");
                             },
                             on_focus_fn("max_file_size_mb"));
    auto f_flush = InputField("Flush Interval (ms)", &cfg->flush_interval_ms, [](const std::string& s)
                              {
                                  return isPositiveInt(s) ? "" : "Must be positive int";
                              },
                              [on_change]
                              {
                                  on_change("flush_interval_ms");
                              },
                              on_focus_fn("flush_interval_ms"));
    auto f_comp = InputField("Compression Level", &cfg->compression_level, [](const std::string& s)
                             {
                                 return isNonNegInt(s) ? "" : "Must be non-negative int";
                             },
                             [on_change]
                             {
                                 on_change("compression_level");
                             },
...
static Component MakeEpicsReaderForm(EpicsReaderConfig* cfg, PanelAppState* state, ScreenInteractive* /*screen*/)
{
    using namespace wizard_internal;
    auto on_change = [state](const std::string& field)
    {
    ...
    auto f_tp = InputField("Thread Pool", &cfg->thread_pool, [](const std::string& s)
                           ...
                           on_focus_fn("thread_pool"));
    auto f_cbs = InputField("Column Batch Size", &cfg->column_batch_size, [](const std::string& s)
                            {
                                return isPositiveInt(s) ? "" : "Must be positive int";
                            },
                            [on_change]
                            {
                                on_change("column_batch_size");
                            },
                            on_focus_fn("column_batch_size"));

    // epics-base only fields
    auto f_mpt = InputField("Monitor Poll Threads", &cfg->monitor_poll_threads, [](const std::string& s)
                            {
                                return isPositiveInt(s) ? "" : "Must be positive int";
                            },
                            [on_change]
                            {
                                on_change("monitor_poll_threads");
                            },
                            on_focus_fn("monitor_poll_threads"));
    auto f_mpi = InputField("Monitor Poll Interval ms", &cfg->monitor_poll_interval_ms, [](const std::string& s)
                            {
                                return isPositiveInt(s) ? "" : "Must be positive int";
                            },
                            [on_change]
                            {
                                on_change("monitor_poll_interval_ms");
                            },
                            on_focus_fn("monitor_poll_interval_ms"));
    auto base_fields = Container::Vertical({f_mpt, f_mpi});
    auto base_maybe = Maybe(base_fields, [rtype_idx]
                            {
                                return *rtype_idx == 1;
                            });

    // epics-archiver only fields
    auto f_host = InputField("Hostname", &cfg->hostname, [](const std::string& s)
                             {
                                 return s.empty() ? "Must not be empty" : "";
                             },
                             [on_change]
    ...
    auto f_sd = InputField("Start Date", &cfg->start_date, [](const std::string& s)
                           {
                               return (!s.empty() && !isValidIso8601(s)) ? "Invalid ISO8601" : "";
                           },
                           [on_change]
                           {
    ...
    auto f_ed = InputField("End Date", &cfg->end_date, [](const std::string& s)
                           {
                               return (!s.empty() && !isValidIso8601(s)) ? "Invalid ISO8601" : "";
                           },
                           [on_change]
    ...
    auto date_maybe = Maybe(date_fields,
                            [mode_idx]
                            {
                                return *mode_idx == 0 || *mode_idx == 1;
                            });

    auto f_poll = InputField("Poll Interval (s)", &cfg->poll_interval_sec, [](const std::string& s)
                             {
                                 return (!s.empty() && !isPositiveInt(s)) ? "Must be positive int" : "";
                             },
                             [on_change]
                             {
                                 on_change("poll_interval_sec");
                             },
                             on_focus_fn("poll_interval_sec"));
    auto poll_maybe = Maybe(f_poll, [mode_idx]
                            {
                                return *mode_idx == 2;
                            });

    auto f_look = InputField("Lookback (s)", &cfg->lookback_sec, [](const std::string& s)
                             {
                                 return (!s.empty() && !isPositiveInt(s)) ? "Must be positive int" : "";
                             },
                             [on_change]
                             {
                                 on_change("lookback_sec");
                             },
                             on_focus_fn("lookback_sec"));
    auto look_maybe = Maybe(f_look, [mode_idx]
                            {
                                return *mode_idx == 1 || *mode_idx == 2;
                            });

    auto f_cto = InputField("Connect Timeout (s)", &cfg->connect_timeout_sec, [](const std::string& s)
                            {
                                return isPositiveInt(s) ? "" : "Must be positive int";
                            },
                            [on_change]
                            {
                                on_change("connect_timeout_sec");
                            },
                            on_focus_fn("connect_timeout_sec"));
    auto f_tto = InputField("Total Timeout (s)", &cfg->total_timeout_sec, [](const std::string& s)
                            {
                                return isPositiveInt(s) ? "" : "Must be positive int";
                            },
                            [on_change]
    ...
    auto rebuild_meta_labels = [cfg, meta_labels]
    {
        ...
        if (meta_labels->empty())
            meta_labels->push_back("(no metadata)");
    };
    rebuild_meta_labels();
...
static Component MakeQueryableForm(WizardState* w, PanelAppState* state)
{
    ...
    auto f_mldp_iurl = InputField("Ingestion URL", &w->queryable.mldp.ingestion_url, [](const std::string& s)
                                  {
                                      return s.empty() ? "Must not be empty" : "";
                                  },
                                  [on_change]
                                  {
                                      on_change("mldp_ingestion_url");
                                  },
                                  on_focus_fn("mldp_ingestion_url"));
    auto f_mldp_qurl = InputField("Query URL", &w->queryable.mldp.query_url, [](const std::string&)
                                  {
                                      return "";
                                  },
                                  [on_change]
                                  {
                                      on_change("mldp_query_url");
                                  },
                                  on_focus_fn("mldp_query_url"));
    auto f_mldp_minc = InputField("Min Connections", &w->queryable.mldp.min_conn, [](const std::string& s)
                                  {
                                      return isPositiveInt(s) ? "" : "Must be positive int";
                                  },
                                  [on_change]
                                  {
                                      on_change("mldp_min_conn");
                                  },
                                  on_focus_fn("mldp_min_conn"));
    auto f_mldp_maxc = InputField("Max Connections", &w->queryable.mldp.max_conn, [](const std::string& s)
                                  {
                                      return isPositiveInt(s) ? "" : "Must be positive int";
                                  },
                                  [on_change]
                                  {
                                      on_change("mldp_max_conn");
                                  },
                                  on_focus_fn("mldp_max_conn"));
    auto mldp_detail = Container::Vertical({f_mldp_iurl, f_mldp_qurl, f_mldp_minc, f_mldp_maxc});
    auto mldp_maybe = Maybe(mldp_detail, [w]
                            {
                                return w->queryable.mldp.enabled;
                            });
...
static Component MakeMetricsForm(WizardState* w, PanelAppState* state)
{
    ...
    auto f_endpoint = InputField("Endpoint", &w->metrics_endpoint, [](const std::string& s)
                                 {
                                     return s.empty() ? "Must not be empty" : "";
                                 },
                                 [on_change]
    ...
    auto f_interval = InputField("Interval (s)", &w->metrics_interval, [](const std::string& s)
                                 {
                                     return isPositiveInt(s) ? "" : "Must be positive int";
                                 },
                                 [on_change]
    ...
    auto detail_maybe = Maybe(detail, [w]
                              {
                                  return w->metrics_enabled;
                              });
...
static Component MakeRoutingForm(WizardState* w, PanelAppState* state)
{
    ...
    auto manual_maybe = Maybe(manual_info, [w]
                              {
                                  return !w->routing_all_to_all;
                              });
...
int runWizard(const std::string& output_path, const std::string& from_path)
{
    ...
    if (!from_path.empty())
    {
        try
        {
            wizard_internal::loadFromConfig(from_path, wizard);
        }
        catch (...)
        { /* ignore — start fresh */
        }
    ...
    auto empty_group = Renderer([]
                                {
                                    return text("  Press [a] to add item.");
                                });
    ...
    auto final_app = CatchEvent(with_modals, [&](Event ev) -> bool
                                {
                                    ...
                                    if (state.show_add_modal)
                                    {
                                        ...
                                        if (!on_reader)
                                        {
                                            ...
                                            {
                                                MldpWriterConfig c;
                                                c.name = "mldp_" + std::to_string(state.wizard.mldp_writers.size());
