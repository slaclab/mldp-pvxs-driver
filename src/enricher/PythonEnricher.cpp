...
{
    if (util::bus::isTimeSeries(batch)) return "time-series";
    if (util::bus::isSourceMetadata(batch)) return "source-metadata";
    if (util::bus::isConfiguration(batch)) return "configuration";
...
void PythonEnricher::configure(const config::Config& config)
{
    ...
    Py_XDECREF(enrich_function_);
    Py_XDECREF(module_);
    enrich_function_ = nullptr;
    module_ = loadModule(script_path);
    enrich_function_ = PyObject_GetAttrString(module_, "enrich");
    if (enrich_function_ == nullptr || !PyCallable_Check(enrich_function_))
    {
        Py_XDECREF(enrich_function_);
        enrich_function_ = nullptr;
        Py_DECREF(module_);
