...
std::vector<IChannelProcessorUPtr> loadScripts(
    ...
    const std::string&                        name_override = "")
{
    ...
    ensurePythonReady();
    ensureScriptDirOnPath(import_dir);
    ...
    std::shared_ptr<BS::light_thread_pool> thread_pool)
{
    ...
    return loadScripts(scripts, script_dir, std::move(bus), std::move(metrics), std::move(thread_pool));
}

std::vector<IChannelProcessorUPtr> PythonScriptDirectoryLoader::loadScript(
...
static bool reg_python_script = ChannelProcessorFactory::registerType(
    "python-script",
    [](const config::Config&                  cfg,
       std::shared_ptr<util::bus::IDataBus>   bus,
       std::shared_ptr<metrics::Metrics>      metrics,
       std::shared_ptr<BS::light_thread_pool> thread_pool) -> std::vector<IChannelProcessorUPtr>
    {
        return PythonScriptDirectoryLoader::loadScript(
