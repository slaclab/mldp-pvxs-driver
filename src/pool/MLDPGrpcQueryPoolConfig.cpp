...
// ---------------------------------------------------------------------------
// Construct from YAML (query-url, min-conn, max-conn only)
// ---------------------------------------------------------------------------

MLDPGrpcQueryPoolConfig::MLDPGrpcQueryPoolConfig(const Config& node)
{
    if (!node.valid())
        throw Error("query-pool: configuration node is invalid");

    const std::string url = node.get(QueryUrlKey, "");
    if (url.empty())
        throw Error("query-pool: '" + std::string(QueryUrlKey) + "' is required");

    if (!node.hasChild(MinConnKey))
        throw Error("query-pool: '" + std::string(MinConnKey) + "' is required");
    const int min_conn = node.getInt(MinConnKey);
    if (min_conn <= 0)
        throw Error("query-pool: '" + std::string(MinConnKey) + "' must be > 0");

    if (!node.hasChild(MaxConnKey))
        throw Error("query-pool: '" + std::string(MaxConnKey) + "' is required");
    const int max_conn = node.getInt(MaxConnKey);
    if (max_conn <= 0)
        throw Error("query-pool: '" + std::string(MaxConnKey) + "' must be > 0");
    if (max_conn < min_conn)
        throw Error("query-pool: max-conn must be >= min-conn");
    ...
    if (node.hasChild(CredentialsKey))
    {
        ...
        if (nodes.empty())
            throw Error("query-pool: 'credentials' is present but empty");
        ...
        if (!tree.is_map())
        {
            ...
            if (t == "ssl")
                ...
                throw Error("query-pool: 'credentials' must be 'none', 'ssl', or a TLS map");
        }
        else
        {
            creds.type = Credentials::Type::Ssl;
            auto readFile = [](const std::string& path) -> std::string {
                std::ifstream f(path);
                if (!f.is_open())
                    throw Error("query-pool: cannot read file '" + path + "'");
...
MLDPGrpcQueryPoolConfig::MLDPGrpcQueryPoolConfig(const MLDPGrpcPoolConfig& full)
{
    if (full.queryUrl().empty())
        throw Error("MLDPGrpcQueryPoolConfig: query-url is empty in source pool config");
